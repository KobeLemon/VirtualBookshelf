import { UserUser } from "@/app/lib/definitions";
...
export default function AccountCreation({name, email, pictures } : UserUser) {
	...
	const formSubmission = async(e:any) => {
		...
		try{
			const results = await fetch('/api/createUser', {
				...
				body: JSON.stringify({User: userData, User: userData})
			})
